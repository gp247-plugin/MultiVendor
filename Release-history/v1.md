...
- Many sellers on **one domain**: a shop lives at `/shop/{code}`, a directory at `/shop`, the cart is split per shop and **each shop gets its own order**. The marketplace collects the money and pays each shop its share per period.
